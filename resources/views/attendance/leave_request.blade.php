...
<div class="content-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <small class="text-muted">Submit a new full day, half day, or short leave request.</small>
        </div>
    ...
    <form action="{{ route('attendance.leave_request.store') }}" method="POST">
        @csrf

        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">Employee</label>
                <input type="text" id="employee-search" list="employee-options" name="employee_search" class="form-control mb-2" placeholder="Type to search employee..." autocomplete="off" value="{{ old('employee_search') }}">
                <datalist id="employee-options">
                    @foreach($employees as $employee)
                        <option value="{{ $employee->employee_name }} ({{ $employee->employee_code }})" data-id="{{ $employee->id }}"></option>
                    @endforeach
                </datalist>
                <input type="hidden" name="employee_id" id="employee-id" value="{{ old('employee_id') }}">
            ...
            <div class="col-12">
                ...
                <textarea name="reason" rows="4" class="form-control" placeholder="Optional reason for the request">{{ old('reason') }}</textarea>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (employeeSearch && employeeIdInput) {
            // When user types, update the hidden employee_id when exact match is found
