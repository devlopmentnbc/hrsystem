...
@section('title', 'Edit Shift Group')
...
<div class="content-card">
    ...
    <form
        method="POST"
        action="{{ route('shifts_groups.update', $shifts_group) }}"
    >
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-4">
                ...
                <input
                    ...
                    name="group_name"
                    class="form-control"
                    value="{{ old('group_name', $shifts_group->group_name) }}"
                    required
                >
            ...
            <div class="col-md-6 mb-4">
                ...
                <select name="category" id="category-select" class="form-select">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category }}" {{ old('category', $shifts_group->category) === $category ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
            ...
            <div class="col-md-4 mb-4">
                ...
                <select
                    ...
                    class="form-select"
                >
                    <option value="1" {{ old('status', $shifts_group->status) == '1' ? 'selected' : '' }}>
                        Active
                    </option>
                    <option value="0" {{ old('status', $shifts_group->status) == '0' ? 'selected' : '' }}>
                        Inactive
                    </option>
                </select>
            ...
            <div class="col-md-4 mb-4">
                ...
                >
                <div class="form-text">The selected employees will belong to this group starting from this date.</div>
            ...
            <div class="col-md-4 mb-4">
                ...
                >
                <div class="form-text">Leave blank to keep this membership active until a future change closes it.</div>
            ...
            <div class="col-12 mb-4">
                ...
                >
                    @foreach($employees->whereIn('id', old('employee_ids', $selectedEmployeeIds ?? [])) as $selectedEmployee)
                        <span class="badge bg-primary text-white selected-employee-item d-inline-flex align-items-center" data-id="{{ $selectedEmployee->id }}">
                            ...
                            <input type="hidden" name="employee_ids[]" value="{{ $selectedEmployee->id }}" data-employee-id="{{ $selectedEmployee->id }}">
                        </span>
                    @endforeach
                </div>

                <div class="form-text">
                    Type or choose an employee, then click Add. Saving replaces this group's assignments for the chosen effective period and preserves older history outside that period.
                </div>
            ...
            <div class="col-12 mb-4">
                ...
                <textarea
                    ...
                    class="form-control"
                    rows="4"
                >{{ old('remarks', $shifts_group->remarks) }}</textarea>
            ...
            <div class="col-12 mb-4">
                <label class="form-label fw-semibold">Assignment History</label>
                    ...
                    <table class="table table-sm mb-0">
                        ...
                        <tbody>
                            @forelse($assignmentHistory as $assignment)
                                <tr>
                                    <td>{{ $assignment->employee?->employee_name }} ({{ $assignment->employee?->employee_code }})</td>
                                    <td>{{ $assignment->effective_start_date?->toDateString() ?? '-' }}</td>
                                    <td>{{ $assignment->effective_end_date?->toDateString() ?? 'Open' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center text-muted">No assignment history recorded yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="d-flex gap-3">
            ...
            <a href="{{ route('shifts_groups.index') }}" class="btn btn-secondary rounded-pill px-5">
                Cancel
            </a>
        </div>
...
<script>
    (function () {
        ...
        const selectedIds = new Set(@json(old('employee_ids', $selectedEmployeeIds ?? [])));
